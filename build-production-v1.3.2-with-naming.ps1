...
if ($LASTEXITCODE -eq 0) {
    ...
    # Copy APK to builds directory with new naming convention
    $apkSource = "android/app/build/outputs/apk/release/app-release.apk"
    $apkDestination = "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.apk"
...
if ($LASTEXITCODE -eq 0) {
    ...
    # Copy AAB to builds directory with new naming convention
    $aabSource = "android/app/build/outputs/bundle/release/app-release.aab"
    $aabDestination = "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.aab"
...
if (Test-Path "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.apk") {
    $apkSize = [math]::Round((Get-Item "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.apk").Length / 1MB, 2)
    Write-Host "   ✅ APK: Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.apk ($apkSize MB)" -ForegroundColor Green
}
if (Test-Path "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.aab") {
    $aabSize = [math]::Round((Get-Item "$BUILDS_DIR/Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.aab").Length / 1MB, 2)
    Write-Host "   ✅ AAB: Whispr_v1.32_v${VERSION_CODE}_${BUILD_DATE}.aab ($aabSize MB)" -ForegroundColor Green
}
