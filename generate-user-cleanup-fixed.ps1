# User Cleanup Script Generator - FIXED VERSION
...
-- ==============================================
-- CONFIGURATION - CHANGE THIS EMAIL
-- ==============================================
-- Change this email address to target the user you want to clean up
...
-- SELECT 
--     CASE 
--         WHEN EXISTS(SELECT 1 FROM public.user_profiles WHERE email = :'target_email') 
--         THEN 'User profile still exists'
--         ELSE 'User profile deleted successfully'
--     END as user_profile_status,
--     CASE 
--         WHEN EXISTS(SELECT 1 FROM auth.users WHERE email = :'target_email') 
