...
class PermissionService {
  ...
  /**
   * Open app permissions settings page (shows Permissions option prominently)
   */
  openAppPermissionsSettings(): void {
    console.log('🔧 openAppPermissionsSettings called');
    console.log('🔧 Platform.OS:', Platform.OS);
    console.log('🔧 PermissionModule available:', !!PermissionModule);
    
    if (Platform.OS === 'android') {
      // Try native method first (requires app rebuild to work)
      if (PermissionModule && typeof PermissionModule.openAppPermissionsSettings === 'function') {
        console.log('✅ Using native method...');
        try {
          PermissionModule.openAppPermissionsSettings();
          console.log('✅ Native method executed');
          return;
        } catch (nativeError) {
          console.error('❌ Native method error:', nativeError);
        }
      } else {
        console.log('⚠️ Native method not available');
        if (PermissionModule) {
          console.log('⚠️ PermissionModule methods:', Object.keys(PermissionModule));
        }
      }
      
      // Fallback: Use Linking.openSettings() which opens app-specific settings on Android
      console.log('🔄 Using Linking.openSettings()...');
