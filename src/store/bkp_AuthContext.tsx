...
import { AppState, AppStateStatus, Alert, Platform, NativeModules, Linking } from 'react-native';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const checkNotificationPermissions = async () => {
    try {
      ...
      if (isAuthorized) {
        ...
        // Ensure FCM token is saved if user is authenticated
        if (state.user?.id) {
          try {
      ...
      // Check if we've prompted in this session (to avoid spamming on every foreground)
      // But allow prompting again on app restart if permission is still denied
      // Wait a bit to ensure the flag was cleared on app start (if app was just restarted)
      await new Promise(resolve => setTimeout(resolve, 200));
      const sessionPrompted = await AsyncStorage.getItem('notifPromptedThisSession');
      if (sessionPrompted) {
        console.log('🔔 Notification permission was requested in this session - will not prompt again');
        return;
      }

      // Mark that we've prompted in this session
      await AsyncStorage.setItem('notifPromptedThisSession', 'true');
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                if (NativeModules.PermissionModule && typeof NativeModules.PermissionModule.openBatteryOptimizationSettings === 'function') {
                  // Native method returns a Promise automatically
                  try {
                    await NativeModules.PermissionModule.openBatteryOptimizationSettings();
                    console.log('✅ Battery optimization settings opened');
                  } catch (error) {
                    console.error('❌ Error opening battery settings via native method:', error);
                    // Fallback to Linking.openSettings()
                    Linking.openSettings().catch((err) => {
                      console.error('❌ Linking.openSettings() fallback also failed:', err);
                    });
                  }
                } else {
                  console.warn('PermissionModule not available, using Linking.openSettings()');
                  Linking.openSettings().catch((error) => {
                    console.error('❌ Linking.openSettings() failed:', error);
                  });
                }
              } catch (error) {
                ...
                // Fallback to Linking.openSettings()
                Linking.openSettings().catch((err) => {
                  console.error('❌ Linking.openSettings() fallback failed:', err);
                });
              }
            }
          }
  ...
  useEffect(() => {
    // Clear session flag on app start to allow prompting again on app restart
    // Do this immediately to ensure it's cleared before any permission checks
    (async () => {
      try {
