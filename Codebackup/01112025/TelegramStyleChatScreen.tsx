...
export const TelegramStyleChatScreen: React.FC<ChatScreenProps> = ({
  ...
  buddyId: propsBuddyId,
  buddyName: propsBuddyName,
  buddyPromise,
}) => {
  const { theme } = useTheme();
  const styles = createStyles(theme);
  ...
  // Initialize with buddy if available, otherwise null
  const [resolvedBuddy, setResolvedBuddy] = useState(buddy || null);
  
  // Immediate effect to sync resolvedBuddy with buddy prop when it changes
  // This ensures normal navigation (with buddy prop) works immediately
  useEffect(() => {
    if (buddy && buddy.id) {
      // Only update if different to avoid unnecessary re-renders
      if (!resolvedBuddy || resolvedBuddy.id !== buddy.id) {
        console.log('💬 Chat: Buddy prop available, syncing to resolvedBuddy immediately:', buddy.id, buddy.name);
        setResolvedBuddy(buddy);
      }
    }
  }, [buddy?.id, resolvedBuddy?.id]); // Sync when buddy prop changes or resolvedBuddy changes

  // Resolve buddy from promise or lookup if not provided
  useEffect(() => {
    console.log('💬 Chat: Buddy resolution effect triggered', {
      hasBuddy: !!buddy,
      hasBuddyId: !!propsBuddyId,
      hasBuddyName: !!propsBuddyName,
      hasBuddyPromise: !!buddyPromise,
      hasUser: !!user,
      userId: user?.id,
      buddyId: buddy?.id,
    ...
    // Priority 1: If buddy is already provided, use it immediately (normal navigation)
    // Skip resolution if we already have a valid resolvedBuddy matching the prop
    if (buddy && buddy.id) {
      if (!resolvedBuddy || resolvedBuddy.id !== buddy.id) {
        console.log('💬 Chat: Using provided buddy:', buddy.id, buddy.name);
        setResolvedBuddy(buddy);
      }
      return; // Don't try other resolution methods if buddy is provided
    }

    // Priority 2: If buddyPromise is provided, resolve it (async notification navigation)
    if (buddyPromise) {
      console.log('💬 Chat: Resolving buddy from promise...');
      buddyPromise.then((foundBuddy) => {
        if (foundBuddy && foundBuddy.id) {
          console.log('💬 Chat: Resolved buddy from promise:', foundBuddy.id, foundBuddy.name);
          setResolvedBuddy(foundBuddy);
        } else {
          console.warn('💬 Chat: Buddy promise resolved but no buddy found');
        }
      }).catch((error) => {
        console.warn('💬 Chat: Failed to resolve buddy from promise:', error);
      });
      return; // Don't try other methods if promise is provided
    }

    // Priority 3: If we have buddyId or buddyName but no buddy, try to resolve it (notification navigation fallback)
    if ((propsBuddyId || propsBuddyName) && !buddy) {
      console.log('💬 Chat: Starting buddy lookup', { propsBuddyId, propsBuddyName });
      // Don't wait for user - it might not be available yet
      (async () => {
        try {
          // Wait for session
          ...
          if (!sessionReady || !userId) {
            console.warn('💬 Chat: Session not ready, cannot resolve buddy');
            // Try using user from props as fallback
            if (user?.id) {
              userId = user.id;
            } else {
              return;
          ...
          // Try lookup by buddyId first
          if (propsBuddyId && userId) {
            try {
              const { supabase: supabaseClient } = await import('@/config/supabase');
              const { data: buddyData, error: buddyError } = await supabaseClient
                .from('buddies')
                .select('*')
                .eq('id', propsBuddyId)
                .eq('user_id', userId)
              ...
              if (!buddyError && buddyData) {
                ...
                console.log('💬 Chat: Resolved buddy from direct DB lookup:', foundBuddy);
              }
            } catch (error) {
              console.warn('💬 Chat: Direct DB lookup failed:', error);
            }
          }

          // Fallback to cache lookup
          if (!foundBuddy && userId) {
            try {
              ...
              if (propsBuddyId) {
                foundBuddy = buddies.find(b => b.id === propsBuddyId);
              } else if (propsBuddyName) {
                foundBuddy = buddies.find(b => b.name === propsBuddyName || b.username === propsBuddyName);
              }
              if (foundBuddy) {
                console.log('💬 Chat: Resolved buddy from cache lookup:', foundBuddy);
              }
            } catch (error) {
              console.warn('💬 Chat: Cache lookup failed:', error);
            }
          }
          ...
          if (foundBuddy) {
            ...
            console.warn('💬 Chat: Could not resolve buddy:', { propsBuddyId, propsBuddyName, userId });
            // If we can't resolve and we have navigation, go back to buddies
            if (fromNotification && onNavigate) {
              setTimeout(() => {
                console.log('💬 Chat: Navigating back to buddies - could not resolve buddy');
                onNavigate('buddies');
              }, 3000); // Wait 3 seconds before giving up
            }
          }
        } catch (error) {
          console.error('💬 Chat: Error resolving buddy:', error);
          // Fallback navigation if from notification
          if (fromNotification && onNavigate) {
            setTimeout(() => {
              console.log('💬 Chat: Navigating back to buddies - error resolving');
              onNavigate('buddies');
            }, 3000);
          }
        }
      ...
      // No buddy info at all - this shouldn't happen in normal flow
      console.warn('💬 Chat: No buddy, buddyId, buddyName, or buddyPromise provided');
    }
  }, [buddy, propsBuddyId, propsBuddyName, buddyPromise, user?.id, fromNotification, onNavigate]);

  // Use resolved buddy for all operations (prioritize resolved, fallback to prop)
  const currentBuddy = resolvedBuddy || buddy;
  
  // Debug log to track current state
  useEffect(() => {
    console.log('💬 Chat: currentBuddy state updated', {
      hasResolvedBuddy: !!resolvedBuddy,
      hasBuddyProp: !!buddy,
      hasCurrentBuddy: !!currentBuddy,
      currentBuddyId: currentBuddy?.id,
      currentBuddyName: currentBuddy?.name,
    });
  }, [resolvedBuddy, buddy, currentBuddy]);

  // Load messages on component mount and when buddy changes
  useEffect(() => {
    if (currentBuddy?.id && user?.id) {
      // If navigating from notification, invalidate cache and force fresh data
      if (fromNotification) {
        console.log('🔄 Loading fresh messages from notification navigation');
        // Invalidate cache for this buddy to ensure fresh data (async IIFE)
        (async () => {
          try {
            const { QueryCache } = await import('@/services/queryCache');
            QueryCache.invalidateMessages(currentBuddy.id);
            console.log('✅ Cache invalidated for notification navigation');
          } catch (error) {
            console.warn('⚠️ Could not invalidate cache:', error);
          }
        })();
        // Force refresh with loading indicator
        loadMessages(true, false); // isRefresh=true, isSilent=false
      } else {
        // Normal flow with cache-first
        loadMessages();
      }
      ...
      const markAsRead = async () => {
        try {
          const { CachedBuddiesService } = await import('@/services/cachedBuddiesService');
          console.log('📖 Marking messages as read for buddy:', currentBuddy.id);
          await CachedBuddiesService.markMessagesAsRead(currentBuddy.id, user.id);
          console.log('✅ Messages marked as read');
          
          // Clear notification batch for this user AFTER messages are loaded
          // This prevents missing notifications if more messages arrive during navigation
          try {
            ...
            if (currentBuddy.name) {
              phase3Service.clearUserBatch(currentBuddy.name);
              console.log('🧠 Cleared notification batch for:', currentBuddy.name);
            }
          } catch (batchError) {
          ...
          if (onMessagesRead) {
            onMessagesRead(currentBuddy.id);
          }
        } catch (error) {
      ...
      // Increased delay to ensure messages are fully loaded before clearing batch
      // This prevents race condition where batch is cleared before all messages arrive
      setTimeout(markAsRead, 1000); // Changed from 500ms to 1000ms
    }
  }, [currentBuddy?.id, user?.id, onMessagesRead, fromNotification]);

  // Set active chat when component mounts and clear when unmounts
  useEffect(() => {
    if (currentBuddy?.id) {
      activeChatService.setActiveChat(currentBuddy.id);
    }
  ...
  useEffect(() => {
    if (!currentBuddy?.id || !user?.id) return;
    ...
    const handleRealtimeMessageUpdate = async (event: any) => {
      try {
        ...
        if (currentBuddy && messageBuddyId === currentBuddy.id) {
  ...
  const handleBuddyDeleted = (eventData: any) => {
    try {
      ...
      if (eventData?.buddyId === currentBuddy.id || eventData?.buddy_user_id === currentBuddy.buddy_user_id) {
  ...
  const loadMessages = async (isRefresh = false, isSilent = false) => {
    try {
      ...
      const chatMessages = await TelegramStyleChatService.getMessages(user.id, currentBuddy.id);
      ...
      try {
        ...
        // Service now returns empty object instead of throwing, but keep this as safety net
        console.warn('Error loading replies (non-critical):', error?.message || error);
        setRepliesByMessageId({}); // Set empty object to prevent undefined issues
      }
  ...
  const addNewMessageIncrementally = (messageData: any) => {
    ...
    const newMessage: SimpleMessage = {
      ...
      chat_id: messageData.buddy_id || currentBuddy?.id || 'unknown',
  ...
  const deleteBuddy = async () => {
    Alert.alert(
      'Delete Buddy',
      `Are you sure you want to delete ${currentBuddy?.name || 'this buddy'}? This will permanently remove the buddy relationship and all messages.`,
      [
        {
          text: 'Cancel',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (!currentBuddy?.id) return;
              const result = await BuddiesService.deleteBuddy(currentBuddy.id, user.id);
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !currentBuddy?.id || !user?.id) return;
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Clear from database - use buddyUserId, not currentBuddy.id
              if (!currentBuddy) return;
              await TelegramStyleChatService.clearChat(user.id, currentBuddy.buddyUserId);
  ...
  const getOtherUserId = () => {
    ...
    const otherUserId = currentBuddy.buddyUserId || currentBuddy.id;
    return otherUserId;
  };

  // Get the other user's name
  const getOtherUserName = () => {
    if (!currentBuddy) return 'Chat';
    
    // Use the buddy's name or display name
    return currentBuddy.name || currentBuddy.display_name || 'Chat';
  };
  ...
  // Show loading state if buddy is not resolved yet
  // Only show loading if we have some info to resolve, otherwise show error
  if (!currentBuddy) {
    const hasBuddyInfo = propsBuddyId || propsBuddyName || buddyPromise;
    
    // Log the loading state for debugging
    console.log('💬 Chat: Showing loading screen - waiting for buddy', {
      hasBuddy: !!buddy,
      hasResolvedBuddy: !!resolvedBuddy,
      hasBuddyId: !!propsBuddyId,
      hasBuddyName: !!propsBuddyName,
      ...
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
        {hasBuddyInfo ? (
          <>
            ...
            <Text style={[styles.errorText, { marginTop: 5, fontSize: 12, color: theme.colors.onSurfaceVariant }]}>
              {propsBuddyId ? `Looking up buddy: ${propsBuddyId.substring(0, 8)}...` : 
               propsBuddyName ? `Looking up buddy: ${propsBuddyName}` : 
               'Resolving buddy...'}
            </Text>
          </>
        ) : (
  ...
  // Check if user is available (warn but don't block if missing)
  if (!user) {
    console.warn('💬 Chat: User not available, but proceeding with buddy:', currentBuddy?.id);
  }
