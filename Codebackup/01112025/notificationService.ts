...
class NotificationServiceClass implements NotificationService {
  ...
  private fcmHandlersSetup = false; // Track if FCM handlers are already set up
  ...
  /**
   * Set up FCM message handlers (consolidated from App.tsx and notificationService)
   * This ensures handlers are registered only once and in a single location
   */
  private setupFCMHandlers(): void {
    ...
    console.log('🔥 Setting up consolidated FCM handlers...');
    
    // Handle background messages (must be set up early, before app initialization)
    // Note: This can only be called once per app lifecycle
    try {
      ...
      // setBackgroundMessageHandler may already be registered, that's okay
      console.log('🔥 Background handler may already be registered');
    }
    
    // Handle foreground messages (includes ping messages)
    messaging().onMessage(async (remoteMessage) => {
      console.log('🔥 FCM message received in foreground:', remoteMessage);
      
      // Check if this is a ping message (Phase 1: Wake-up signal)
      if (remoteMessage.data?.type === 'ping') {
        console.log('🔥 FCM ping received - handling wake-up...');
        await this.handleFCMPing();
        return; // Ping handled, don't process further
      }
      
      // For other message types, skip notification - hybrid system handles it via realtime
    ...
    (async () => {
      try {
        ...
        if (!buddyName && !buddyId) {
          console.warn('🔔 [TAP] No buddyName or buddyId in notification, cannot navigate');
          return;
        }

        console.log('🔔 [TAP] Navigating to chat for:', { buddyName, buddyId });

        // NOTE: Batch clearing moved to TelegramStyleChatScreen
        // Clears after messages are loaded to prevent missing notifications during navigation

        // ✅ EMIT IMMEDIATELY - Don't block on session/buddy lookup
        // AppNavigator will handle async buddy lookup without blocking navigation
        console.log('🔔 [TAP] Emitting navigation event immediately with available info');
        DeviceEventEmitter.emit('navigateToChat', { buddyId, buddyName, fromNotification: true });
      } catch (error) {
