# Whispr Mobile App Production Build Script v1.4.0
...
# File naming convention: Whispr_v1.4.0_v51_2025-Date_Time
...
Files Generated:
- APK: $APK_NAME ($([math]::Round($apkSize, 2)) MB)
- AAB: $AAB_NAME ($([math]::Round($aabSize, 2)) MB)

Features in this version:
- Enhanced AI model with creative text generation
- "Surprise Me!" AI enhancement option
- Improved dark theme support across all screens
- Fixed message replies system
- Enhanced connection recovery
- Safe area context integration
- Comprehensive theme implementation

Deployment Notes:
- AAB file is ready for Play Store upload
- APK file can be used for direct installation
- All builds are signed with release keystore
- Tested on Android API 24+ (Android 7.0+)

Build completed successfully!
"@

$buildInfo | Out-File -FilePath "$BUILDS_DIR/build-info-v${VERSION_NAME}.txt" -Encoding UTF8
$buildInfo | Out-File -FilePath "$LATEST_DIR/build-info-latest.txt" -Encoding UTF8

Write-Host "Build completed successfully!" -ForegroundColor Green
Write-Host "Ready for Play Store deployment" -ForegroundColor Green
Write-Host "Build info saved to: $BUILDS_DIR/build-info-v${VERSION_NAME}.txt" -ForegroundColor Cyan
